...
ITA_mispricing <- fread("C:/Data/Mispricing/ita_mispricing.csv")

setnames(ITA_mispricing, old = c( "me_lag1",   "ret",    "o_score",   "ret_12_1",      "gp_at",  "niq_at",  "chcsho_12m",        "eqnpo_12m",    "oaccruals_at",     "noa_at",          "at_gr1",      "ppeinv_gr1a"),
...
ITA_mispricing_relevant <- ITA_mispricing[,.(id, eom, size_grp, LagMV_USD, RET_USD,
...
ITA_mispricing_relevant[, date := as.yearmon(as.Date(as.character(eom), "%Y%m%d"), "%Y-%m")]

# Create a lagged dataset
ITA_mispricing_lagged <- ITA_mispricing_relevant %>%
...
ITA_mispricing_with_lags <- ITA_mispricing_relevant %>%
  left_join(ITA_mispricing_lagged, by = c("date", "id"))

ITA_mispricing_with_lags_relevant <- ITA_mispricing_with_lags[,.(id, date, size_grp, size_grp_L1, LagMV_USD.x, RET_USD.x,
...
IT_Data_long_ST <- ITA_mispricing_with_lags_relevant %>%
...
IT_Data_long_ST <- IT_Data_long_ST[order(date)]

# Set the required sample period (All anomalies from 199801 to 202012)
IT_Data_long_ST[, date := as.character(format(as.yearmon(date, "%Y-%m"), "%Y%m"))]
IT_Data_long_ST <- IT_Data_long_ST[date >= "199801" & date <= "202012"]
fwrite(IT_Data_long_ST, "C:/Data/Anomalies/IT_Data_long_ST.csv")

################################################# Create Long and Short strategies ###########################################
IT_Data_long_ST <- fread("C:/Data/Anomalies/IT_Data_long_ST.csv")
...
IT_strategies <- createLongShortStrategies(IT_Data_long_ST, allAnomalies, inverseAnomalies)
fwrite(IT_strategies, "C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_ITA.csv")

################################################# Calculate the Excess Return ###########################################
IT_strategies <- fread("C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_ITA.csv")
setDT(IT_strategies)
IT_strategies[, date := as.character(date)]

IT_Merged_Data <- fread("C:/Data/Merged Data/Merged_IT_Data.csv") # include FF3M-factors and categorized Sentiment Index
IT_Merged_Data[, date := as.character(date)]

merged_IT_data <- merge(IT_Merged_Data, IT_strategies, by = "date", all = TRUE)
merged_IT_data <- merged_IT_data[!is.na(date)]

# Calculate excess returns under long strategies and short strategies

cols_to_adjust <- grep("^(Long_|Short_)", names(merged_IT_data), value = TRUE)

for (col in cols_to_adjust) {
  new_col_name <- paste0("ex_", col)
  merged_IT_data[, (new_col_name) := get(col) - RF]
}

# Calculate Spread of excess return under long-short strategies

long_cols <- grep("^ex_Long_", names(merged_IT_data), value = TRUE)
short_cols <- grep("^ex_Short_", names(merged_IT_data), value = TRUE)

for (i in seq_along(long_cols)) {
  spread_name <- sub("^ex_Long_", "ex_Spread_", long_cols[i])
  merged_IT_data[, (spread_name) := get(long_cols[i]) - get(short_cols[i])]
}

cols_to_remove <- grep("^(Long_|Short_|Spread_)", names(merged_IT_data), value = TRUE)
merged_IT_data[, (cols_to_remove) := NULL]

merged_IT_data <- merged_IT_data[,.(date, Mkt.RF, SMB, HML, RF,
...
fwrite(merged_IT_data, "C:/Data/Mispricing/merged_IT_data.csv")
...
data <- read.csv("C:/Data/Mispricing/merged_IT_data.csv", header = TRUE, sep = ",")  
...
addWorksheet(wb, "result1PanelA_IT")
addWorksheet(wb, "result1PanelB_IT")
addWorksheet(wb, "result1PanelC_IT")

writeData(wb, "result1PanelA_IT", formatted_lower_triangular, withFilter = FALSE)
writeData(wb, "result1PanelB_IT", result1B_cleaned, withFilter = FALSE)
writeData(wb, "result1PanelC_IT", result1C_cleaned, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result1_IT.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result2_long_IT")
addWorksheet(wb, "result2_short_IT")
addWorksheet(wb, "result2_spread_IT")

writeData(wb, "result2_long_IT", result2_long, withFilter = FALSE)
writeData(wb, "result2_short_IT", result2_short, withFilter = FALSE)
writeData(wb, "result2_spread_IT", result2_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result2_IT.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result3_long_IT")
addWorksheet(wb, "result3_short_IT")
addWorksheet(wb, "result3_spread_IT")

writeData(wb, "result3_long_IT", result3_long, withFilter = FALSE)
writeData(wb, "result3_short_IT", result3_short, withFilter = FALSE)
writeData(wb, "result3_spread_IT", result3_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result3_IT.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result4_long_IT")
addWorksheet(wb, "result4_short_IT")
addWorksheet(wb, "result4_spread_IT")

writeData(wb, "result4_long_IT", result4_long, withFilter = FALSE)
writeData(wb, "result4_short_IT", result4_short, withFilter = FALSE)
writeData(wb, "result4_spread_IT", result4_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result4_IT.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result5_long_IT")
addWorksheet(wb, "result5_short_IT")
addWorksheet(wb, "result5_spread_IT")

writeData(wb, "result5_long_IT", result5_long, withFilter = FALSE)
writeData(wb, "result5_short_IT", result5_short, withFilter = FALSE)
writeData(wb, "result5_spread_IT", result5_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result5_IT.xlsx", overwrite = TRUE)
