...
JPN_mispricing <- fread("C:/Data/Mispricing/jpn_mispricing.csv")

setnames(JPN_mispricing, old = c( "me_lag1",   "ret",    "o_score",   "ret_12_1",      "gp_at",  "niq_at",  "chcsho_12m",        "eqnpo_12m",    "oaccruals_at",     "noa_at",          "at_gr1",      "ppeinv_gr1a"),
...
JPN_mispricing_relevant <- JPN_mispricing[,.(id, eom, size_grp, LagMV_USD, RET_USD,
...
JPN_mispricing_relevant[, date := as.yearmon(as.Date(as.character(eom), "%Y%m%d"), "%Y-%m")]

# Create a lagged dataset
JPN_mispricing_lagged <- JPN_mispricing_relevant %>%
...
JPN_mispricing_with_lags <- JPN_mispricing_relevant %>%
  left_join(JPN_mispricing_lagged, by = c("date", "id"))

JPN_mispricing_with_lags_relevant <- JPN_mispricing_with_lags[,.(id, date, size_grp, size_grp_L1, LagMV_USD.x, RET_USD.x,
...
JP_Data_long_ST <- JPN_mispricing_with_lags_relevant %>%
...
JP_Data_long_ST <- JP_Data_long_ST[order(date)]

# Set the required sample period (All anomalies from 199801 to 202012)
JP_Data_long_ST[, date := as.character(format(as.yearmon(date, "%Y-%m"), "%Y%m"))]
JP_Data_long_ST <- JP_Data_long_ST[date >= "199801" & date <= "202012"]
fwrite(JP_Data_long_ST, "C:/Data/Anomalies/JP_Data_long_ST.csv")

################################################# Create Long and Short strategies ###########################################
JP_Data_long_ST <- fread("C:/Data/Anomalies/JP_Data_long_ST.csv")
...
JP_strategies <- createLongShortStrategies(JP_Data_long_ST, allAnomalies, inverseAnomalies)
fwrite(JP_strategies, "C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_JPN.csv")

################################################# Calculate the Excess Return ###########################################
JP_strategies <- fread("C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_JPN.csv")
setDT(JP_strategies)
JP_strategies[, date := as.character(date)]

JP_Merged_Data <- fread("C:/Data/Merged Data/Merged_JP_Data.csv") # include FF3M-factors and categorized Sentiment Index
JP_Merged_Data[, date := as.character(date)]

merged_JP_data <- merge(JP_Merged_Data, JP_strategies, by = "date", all = TRUE)
merged_JP_data <- merged_JP_data[!is.na(date)]

# Calculate excess returns under long strategies and short strategies

cols_to_adjust <- grep("^(Long_|Short_)", names(merged_JP_data), value = TRUE)

for (col in cols_to_adjust) {
  new_col_name <- paste0("ex_", col)
  merged_JP_data[, (new_col_name) := get(col) - RF]
}

# Calculate Spread of excess return under long-short strategies

long_cols <- grep("^ex_Long_", names(merged_JP_data), value = TRUE)
short_cols <- grep("^ex_Short_", names(merged_JP_data), value = TRUE)

for (i in seq_along(long_cols)) {
  spread_name <- sub("^ex_Long_", "ex_Spread_", long_cols[i])
  merged_JP_data[, (spread_name) := get(long_cols[i]) - get(short_cols[i])]
}

cols_to_remove <- grep("^(Long_|Short_|Spread_)", names(merged_JP_data), value = TRUE)
merged_JP_data[, (cols_to_remove) := NULL]

merged_JP_data <- merged_JP_data[,.(date, Mkt.RF, SMB, HML, RF,
...
fwrite(merged_JP_data, "C:/Data/Mispricing/merged_JP_data.csv")
...
data <- read.csv("C:/Data/Mispricing/merged_JP_data.csv", header = TRUE, sep = ",")  
...
## Table 1 Panel C: Average benchmark-adjusted returns for 10 anomalies under long-short strategies
...
addWorksheet(wb, "result1PanelA_JP")
addWorksheet(wb, "result1PanelB_JP")
addWorksheet(wb, "result1PanelC_JP")

writeData(wb, "result1PanelA_JP", formatted_lower_triangular, withFilter = FALSE)
writeData(wb, "result1PanelB_JP", result1B_cleaned, withFilter = FALSE)
writeData(wb, "result1PanelC_JP", result1C_cleaned, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result1_JP.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result2_long_JP")
addWorksheet(wb, "result2_short_JP")
addWorksheet(wb, "result2_spread_JP")

writeData(wb, "result2_long_JP", result2_long, withFilter = FALSE)
writeData(wb, "result2_short_JP", result2_short, withFilter = FALSE)
writeData(wb, "result2_spread_JP", result2_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result2_JP.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result3_long_JP")
addWorksheet(wb, "result3_short_JP")
addWorksheet(wb, "result3_spread_JP")

writeData(wb, "result3_long_JP", result3_long, withFilter = FALSE)
writeData(wb, "result3_short_JP", result3_short, withFilter = FALSE)
writeData(wb, "result3_spread_JP", result3_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result3_JP.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result4_long_JP")
addWorksheet(wb, "result4_short_JP")
addWorksheet(wb, "result4_spread_JP")

writeData(wb, "result4_long_JP", result4_long, withFilter = FALSE)
writeData(wb, "result4_short_JP", result4_short, withFilter = FALSE)
writeData(wb, "result4_spread_JP", result4_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result4_JP.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result5_long_JP")
addWorksheet(wb, "result5_short_JP")
addWorksheet(wb, "result5_spread_JP")

writeData(wb, "result5_long_JP", result5_long, withFilter = FALSE)
writeData(wb, "result5_short_JP", result5_short, withFilter = FALSE)
writeData(wb, "result5_spread_JP", result5_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result5_JP.xlsx", overwrite = TRUE)
