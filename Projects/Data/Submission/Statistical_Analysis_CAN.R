...
CAN_mispricing <- fread("C:/Data/Mispricing/can_mispricing.csv")

setnames(CAN_mispricing, old = c( "me_lag1",   "ret",    "o_score",   "ret_12_1",      "gp_at",  "niq_at",  "chcsho_12m",        "eqnpo_12m",    "oaccruals_at",     "noa_at",          "at_gr1",      "ppeinv_gr1a"),
...
CAN_mispricing_relevant <- CAN_mispricing[,.(id, eom, size_grp, LagMV_USD, RET_USD,
...
CAN_mispricing_relevant[, date := as.yearmon(as.Date(as.character(eom), "%Y%m%d"), "%Y-%m")]

# Create a lagged dataset
CAN_mispricing_lagged <- CAN_mispricing_relevant %>%
...
CAN_mispricing_with_lags <- CAN_mispricing_relevant %>%
  left_join(CAN_mispricing_lagged, by = c("date", "id"))

CAN_mispricing_with_lags_relevant <- CAN_mispricing_with_lags[,.(id, date, size_grp, size_grp_L1, LagMV_USD.x, RET_USD.x,
...
CA_Data_long_ST <- CAN_mispricing_with_lags_relevant %>%
...
CA_Data_long_ST <- CA_Data_long_ST[order(date)]

# Set the required sample period (All anomalies from 199801 to 202012)
CA_Data_long_ST[, date := as.character(format(as.yearmon(date, "%Y-%m"), "%Y%m"))]
CA_Data_long_ST <- CA_Data_long_ST[date >= "199801" & date <= "202012"]
fwrite(CA_Data_long_ST, "C:/Data/Anomalies/CA_Data_long_ST.csv")

################################################# Create Long and Short strategies ###########################################
CA_Data_long_ST <- fread("C:/Data/Anomalies/CA_Data_long_ST.csv")
...
CA_strategies <- createLongShortStrategies(CA_Data_long_ST, allAnomalies, inverseAnomalies)
fwrite(CA_strategies, "C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_CAN.csv")

################################################# Calculate the Excess Return ###########################################
CA_strategies <- fread("C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_CAN.csv")
setDT(CA_strategies)
CA_strategies[, date := as.character(date)]

CA_Merged_Data <- fread("C:/Data/Merged Data/Merged_CA_Data.csv") # include FF3M-factors and categorized Sentiment Index
CA_Merged_Data[, date := as.character(date)]

merged_CA_data <- merge(CA_Merged_Data, CA_strategies, by = "date", all = TRUE)
merged_CA_data <- merged_CA_data[!is.na(date)]

# Calculate excess returns under long strategies and short strategies

cols_to_adjust <- grep("^(Long_|Short_)", names(merged_CA_data), value = TRUE)

for (col in cols_to_adjust) {
  new_col_name <- paste0("ex_", col)
  merged_CA_data[, (new_col_name) := get(col) - RF]
}

# Calculate Spread of excess return under long-short strategies

long_cols <- grep("^ex_Long_", names(merged_CA_data), value = TRUE)
short_cols <- grep("^ex_Short_", names(merged_CA_data), value = TRUE)

for (i in seq_along(long_cols)) {
  spread_name <- sub("^ex_Long_", "ex_Spread_", long_cols[i])
  merged_CA_data[, (spread_name) := get(long_cols[i]) - get(short_cols[i])]
}

cols_to_remove <- grep("^(Long_|Short_|Spread_)", names(merged_CA_data), value = TRUE)
merged_CA_data[, (cols_to_remove) := NULL]

merged_CA_data <- merged_CA_data[,.(date, Mkt.RF, SMB, HML, RF,
...
fwrite(merged_CA_data, "C:/Data/Mispricing/merged_CA_data.csv")
...
data <- read.csv("C:/Data/Mispricing/merged_CA_data.csv", header = TRUE, sep = ",")  
...
addWorksheet(wb, "result1PanelA_CA")
addWorksheet(wb, "result1PanelB_CA")
addWorksheet(wb, "result1PanelC_CA")

writeData(wb, "result1PanelA_CA", formatted_lower_triangular, withFilter = FALSE)
writeData(wb, "result1PanelB_CA", result1B_cleaned, withFilter = FALSE)
writeData(wb, "result1PanelC_CA", result1C_cleaned, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result1_CA.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result2_long_CA")
addWorksheet(wb, "result2_short_CA")
addWorksheet(wb, "result2_spread_CA")

writeData(wb, "result2_long_CA", result2_long, withFilter = FALSE)
writeData(wb, "result2_short_CA", result2_short, withFilter = FALSE)
writeData(wb, "result2_spread_CA", result2_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result2_CA.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result3_long_CA")
addWorksheet(wb, "result3_short_CA")
addWorksheet(wb, "result3_spread_CA")

writeData(wb, "result3_long_CA", result3_long, withFilter = FALSE)
writeData(wb, "result3_short_CA", result3_short, withFilter = FALSE)
writeData(wb, "result3_spread_CA", result3_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result3_CA.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result4_long_CA")
addWorksheet(wb, "result4_short_CA")
addWorksheet(wb, "result4_spread_CA")

writeData(wb, "result4_long_CA", result4_long, withFilter = FALSE)
writeData(wb, "result4_short_CA", result4_short, withFilter = FALSE)
writeData(wb, "result4_spread_CA", result4_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result4_CA.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result5_long_CA")
addWorksheet(wb, "result5_short_CA")
addWorksheet(wb, "result5_spread_CA")

writeData(wb, "result5_long_CA", result5_long, withFilter = FALSE)
writeData(wb, "result5_short_CA", result5_short, withFilter = FALSE)
writeData(wb, "result5_spread_CA", result5_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result5_CA.xlsx", overwrite = TRUE)
