...
fwrite(US_Data_long_ST, "C:/Data/Anomalies/US_Data_long_ST.csv")

################################################# Create Long and Short strategies ###########################################
US_Data_long_ST <- fread("C:/Data/Anomalies/US_Data_long_ST.csv")
...
fwrite(US_strategies, "C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_USA.csv")

################################################# Calculate the Excess Return ###########################################
US_strategies <- fread("C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_USA.csv")
setDT(US_strategies)
US_strategies[, date := as.character(date)]

US_Merged_Data <- fread("C:/Data/Merged Data/Merged_US_Data.csv") # include FF3M-factors and categorized Sentiment Index
...
addWorksheet(wb, "result1PanelA_US")
addWorksheet(wb, "result1PanelB_US")
addWorksheet(wb, "result1PanelC_US")

writeData(wb, "result1PanelA_US", formatted_lower_triangular, withFilter = FALSE)
writeData(wb, "result1PanelB_US", result1B_cleaned, withFilter = FALSE)
writeData(wb, "result1PanelC_US", result1C_cleaned, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result1_US.xlsx", overwrite = TRUE)
...
perform_t_test <- function(colx) {  
  ...
  for (i in seq_along(colx)) {
    ...
    # Correlation calculations for Sentiment and Attention
    corr_hsent <- cor(data$HSENT, data[[colx[i]]], use = "complete.obs")
    corr_lsent <- cor(data$LSENT, data[[colx[i]]], use = "complete.obs")
    corr_hatten <- cor(data$HATTEN, data[[colx[i]]], use = "complete.obs")
    corr_latten <- cor(data$LATTEN, data[[colx[i]]], use = "complete.obs")
    
    # Print results
    cat("\nCorrelations for", colx[i], ":\n")
    cat("Correlation with High Sentiment: ", corr_hsent, "\n")
    cat("Correlation with Low Sentiment: ", corr_lsent, "\n")
    cat("Correlation with High Attention: ", corr_hatten, "\n")
    cat("Correlation with Low Attention: ", corr_latten, "\n")
...
addWorksheet(wb, "result2_long_US")
addWorksheet(wb, "result2_short_US")
addWorksheet(wb, "result2_spread_US")

writeData(wb, "result2_long_US", result2_long, withFilter = FALSE)
writeData(wb, "result2_short_US", result2_short, withFilter = FALSE)
writeData(wb, "result2_spread_US", result2_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result2_US.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result3_long_US")
addWorksheet(wb, "result3_short_US")
addWorksheet(wb, "result3_spread_US")

writeData(wb, "result3_long_US", result3_long, withFilter = FALSE)
writeData(wb, "result3_short_US", result3_short, withFilter = FALSE)
writeData(wb, "result3_spread_US", result3_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result3_US.xlsx", overwrite = TRUE)
...
perform_lm <- function(colx) {
  ...
  for (i in seq_along(colx)) {
    ...
    # Compute correlation of Sentiment and returns
    corr_sent <- cor(data[[colx[i]]], data$LAG_SENT, use = "complete.obs")
    
    cat("\nCorrelations for", colx[i], ":\n")
    cat("Correlation with LAG_SENT: ", corr_sent, "\n")
...
addWorksheet(wb, "result4_long_US")
addWorksheet(wb, "result4_short_US")
addWorksheet(wb, "result4_spread_US")

writeData(wb, "result4_long_US", result4_long, withFilter = FALSE)
writeData(wb, "result4_short_US", result4_short, withFilter = FALSE)
writeData(wb, "result4_spread_US", result4_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result4_US.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result5_long_US")
addWorksheet(wb, "result5_short_US")
addWorksheet(wb, "result5_spread_US")

writeData(wb, "result5_long_US", result5_long, withFilter = FALSE)
writeData(wb, "result5_short_US", result5_short, withFilter = FALSE)
writeData(wb, "result5_spread_US", result5_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result5_US.xlsx", overwrite = TRUE)
