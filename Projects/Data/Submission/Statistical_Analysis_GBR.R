...
GBR_mispricing <- fread("C:/Data/Mispricing/gbr_mispricing.csv")

setnames(GBR_mispricing, old = c( "me_lag1",   "ret",    "o_score",   "ret_12_1",      "gp_at",  "niq_at",  "chcsho_12m",        "eqnpo_12m",    "oaccruals_at",     "noa_at",          "at_gr1",      "ppeinv_gr1a"),
...
GBR_mispricing_relevant <- GBR_mispricing[,.(id, eom, size_grp, LagMV_USD, RET_USD,
...
GBR_mispricing_relevant[, date := as.yearmon(as.Date(as.character(eom), "%Y%m%d"), "%Y-%m")]

# Create a lagged dataset
GBR_mispricing_lagged <- GBR_mispricing_relevant %>%
...
GBR_mispricing_with_lags <- GBR_mispricing_relevant %>%
  left_join(GBR_mispricing_lagged, by = c("date", "id"))

GBR_mispricing_with_lags_relevant <- GBR_mispricing_with_lags[,.(id, date, size_grp, size_grp_L1, LagMV_USD.x, RET_USD.x,
...
GB_Data_long_ST <- GBR_mispricing_with_lags_relevant %>%
...
GB_Data_long_ST <- GB_Data_long_ST[order(date)]

# Set the required sample period (All anomalies from 199801 to 202012)
GB_Data_long_ST[, date := as.character(format(as.yearmon(date, "%Y-%m"), "%Y%m"))]
GB_Data_long_ST <- GB_Data_long_ST[date >= "199801" & date <= "202012"]
fwrite(GB_Data_long_ST, "C:/Data/Anomalies/GB_Data_long_ST.csv")

################################################# Create Long and Short strategies ###########################################
GB_Data_long_ST <- fread("C:/Data/Anomalies/GB_Data_long_ST.csv")
...
GB_strategies <- createLongShortStrategies(GB_Data_long_ST, allAnomalies, inverseAnomalies)
fwrite(GB_strategies, "C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_GBR.csv")

################################################# Calculate the Excess Return ###########################################
GB_strategies <- fread("C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_GBR.csv")
setDT(GB_strategies)
GB_strategies[, date := as.character(date)]

GB_Merged_Data <- fread("C:/Data/Merged Data/Merged_GB_Data.csv") # include FF3M-factors and categorized Sentiment Index
GB_Merged_Data[, date := as.character(date)]

merged_GB_data <- merge(GB_Merged_Data, GB_strategies, by = "date", all = TRUE)
merged_GB_data <- merged_GB_data[!is.na(date)]

# Calculate excess returns under long strategies and short strategies

cols_to_adjust <- grep("^(Long_|Short_)", names(merged_GB_data), value = TRUE)

for (col in cols_to_adjust) {
  new_col_name <- paste0("ex_", col)
  merged_GB_data[, (new_col_name) := get(col) - RF]
}

# Calculate Spread of excess return under long-short strategies

long_cols <- grep("^ex_Long_", names(merged_GB_data), value = TRUE)
short_cols <- grep("^ex_Short_", names(merged_GB_data), value = TRUE)

for (i in seq_along(long_cols)) {
  spread_name <- sub("^ex_Long_", "ex_Spread_", long_cols[i])
  merged_GB_data[, (spread_name) := get(long_cols[i]) - get(short_cols[i])]
}

cols_to_remove <- grep("^(Long_|Short_|Spread_)", names(merged_GB_data), value = TRUE)
merged_GB_data[, (cols_to_remove) := NULL]

merged_GB_data <- merged_GB_data[,.(date, Mkt.RF, SMB, HML, RF,
...
fwrite(merged_GB_data, "C:/Data/Mispricing/merged_GB_data.csv")
...
data <- read.csv("C:/Data/Mispricing/merged_GB_data.csv", header = TRUE, sep = ",")  
...
addWorksheet(wb, "result1PanelA_GB")
addWorksheet(wb, "result1PanelB_GB")
addWorksheet(wb, "result1PanelC_GB")

writeData(wb, "result1PanelA_GB", formatted_lower_triangular, withFilter = FALSE)
writeData(wb, "result1PanelB_GB", result1B_cleaned, withFilter = FALSE)
writeData(wb, "result1PanelC_GB", result1C_cleaned, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result1_GB.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result2_long_GB")
addWorksheet(wb, "result2_short_GB")
addWorksheet(wb, "result2_spread_GB")

writeData(wb, "result2_long_GB", result2_long, withFilter = FALSE)
writeData(wb, "result2_short_GB", result2_short, withFilter = FALSE)
writeData(wb, "result2_spread_GB", result2_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result2_GB.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result3_long_GB")
addWorksheet(wb, "result3_short_GB")
addWorksheet(wb, "result3_spread_GB")

writeData(wb, "result3_long_GB", result3_long, withFilter = FALSE)
writeData(wb, "result3_short_GB", result3_short, withFilter = FALSE)
writeData(wb, "result3_spread_GB", result3_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result3_GB.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result4_long_GB")
addWorksheet(wb, "result4_short_GB")
addWorksheet(wb, "result4_spread_GB")

writeData(wb, "result4_long_GB", result4_long, withFilter = FALSE)
writeData(wb, "result4_short_GB", result4_short, withFilter = FALSE)
writeData(wb, "result4_spread_GB", result4_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result4_GB.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result5_long_GB")
addWorksheet(wb, "result5_short_GB")
addWorksheet(wb, "result5_spread_GB")

writeData(wb, "result5_long_GB", result5_long, withFilter = FALSE)
writeData(wb, "result5_short_GB", result5_short, withFilter = FALSE)
writeData(wb, "result5_spread_GB", result5_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result5_GB.xlsx", overwrite = TRUE)
