...
DEU_mispricing <- fread("C:/Data/Mispricing/deu_mispricing.csv")

setnames(DEU_mispricing, old = c( "me_lag1",   "ret",    "o_score",   "ret_12_1",      "gp_at",  "niq_at",  "chcsho_12m",        "eqnpo_12m",    "oaccruals_at",     "noa_at",          "at_gr1",      "ppeinv_gr1a"),
...
DEU_mispricing_relevant <- DEU_mispricing[,.(id, eom, size_grp, LagMV_USD, RET_USD,
...
DEU_mispricing_relevant[, date := as.yearmon(as.Date(as.character(eom), "%Y%m%d"), "%Y-%m")]

# Create a lagged dataset
DEU_mispricing_lagged <- DEU_mispricing_relevant %>%
...
DEU_mispricing_with_lags <- DEU_mispricing_relevant %>%
  left_join(DEU_mispricing_lagged, by = c("date", "id"))

DEU_mispricing_with_lags_relevant <- DEU_mispricing_with_lags[,.(id, date, size_grp, size_grp_L1, LagMV_USD.x, RET_USD.x,
...
DE_Data_long_ST <- DEU_mispricing_with_lags_relevant %>%
...
DE_Data_long_ST <- DE_Data_long_ST[order(date)]

# Set the required sample period (All anomalies from 199801 to 202012)
DE_Data_long_ST[, date := as.character(format(as.yearmon(date, "%Y-%m"), "%Y%m"))]
DE_Data_long_ST <- DE_Data_long_ST[date >= "199801" & date <= "202012"]
fwrite(DE_Data_long_ST, "C:/Data/Anomalies/DE_Data_long_ST.csv")

################################################# Create Long and Short strategies ###########################################
DE_Data_long_ST <- fread("C:/Data/Anomalies/DE_Data_long_ST.csv")
...
DE_strategies <- createLongShortStrategies(DE_Data_long_ST, allAnomalies, inverseAnomalies)
fwrite(DE_strategies, "C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_DEU.csv")

################################################# Calculate the Excess Return ###########################################
DE_strategies <- fread("C:/Data/Strategies_SI_FF3M/AnomalyRet_10+1_Long_Short_Spread_DEU.csv")
setDT(DE_strategies)
DE_strategies[, date := as.character(date)]

DE_Merged_Data <- fread("C:/Data/Merged Data/Merged_DE_Data.csv") # include FF3M-factors and categorized Sentiment Index
DE_Merged_Data[, date := as.character(date)]

merged_DE_data <- merge(DE_Merged_Data, DE_strategies, by = "date", all = TRUE)
merged_DE_data <- merged_DE_data[!is.na(date)]

# Calculate excess returns under long strategies and short strategies

cols_to_adjust <- grep("^(Long_|Short_)", names(merged_DE_data), value = TRUE)

for (col in cols_to_adjust) {
  new_col_name <- paste0("ex_", col)
  merged_DE_data[, (new_col_name) := get(col) - RF]
}

# Calculate Spread of excess return under long-short strategies

long_cols <- grep("^ex_Long_", names(merged_DE_data), value = TRUE)
short_cols <- grep("^ex_Short_", names(merged_DE_data), value = TRUE)

for (i in seq_along(long_cols)) {
  spread_name <- sub("^ex_Long_", "ex_Spread_", long_cols[i])
  merged_DE_data[, (spread_name) := get(long_cols[i]) - get(short_cols[i])]
}

cols_to_remove <- grep("^(Long_|Short_|Spread_)", names(merged_DE_data), value = TRUE)
merged_DE_data[, (cols_to_remove) := NULL]

merged_DE_data <- merged_DE_data[,.(date, Mkt.RF, SMB, HML, RF,
...
fwrite(merged_DE_data, "C:/Data/Mispricing/merged_DE_data.csv")
...
data <- read.csv("C:/Data/Mispricing/merged_DE_data.csv", header = TRUE, sep = ",")  
...
addWorksheet(wb, "result1PanelA_DE")
addWorksheet(wb, "result1PanelB_DE")
addWorksheet(wb, "result1PanelC_DE")

writeData(wb, "result1PanelA_DE", formatted_lower_triangular, withFilter = FALSE)
writeData(wb, "result1PanelB_DE", result1B_cleaned, withFilter = FALSE)
writeData(wb, "result1PanelC_DE", result1C_cleaned, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result1_DE.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result2_long_DE")
addWorksheet(wb, "result2_short_DE")
addWorksheet(wb, "result2_spread_DE")

writeData(wb, "result2_long_DE", result2_long, withFilter = FALSE)
writeData(wb, "result2_short_DE", result2_short, withFilter = FALSE)
writeData(wb, "result2_spread_DE", result2_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result2_DE.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result3_long_DE")
addWorksheet(wb, "result3_short_DE")
addWorksheet(wb, "result3_spread_DE")

writeData(wb, "result3_long_DE", result3_long, withFilter = FALSE)
writeData(wb, "result3_short_DE", result3_short, withFilter = FALSE)
writeData(wb, "result3_spread_DE", result3_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result3_DE.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result4_long_DE")
addWorksheet(wb, "result4_short_DE")
addWorksheet(wb, "result4_spread_DE")

writeData(wb, "result4_long_DE", result4_long, withFilter = FALSE)
writeData(wb, "result4_short_DE", result4_short, withFilter = FALSE)
writeData(wb, "result4_spread_DE", result4_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result4_DE.xlsx", overwrite = TRUE)
...
addWorksheet(wb, "result5_long_DE")
addWorksheet(wb, "result5_short_DE")
addWorksheet(wb, "result5_spread_DE")

writeData(wb, "result5_long_DE", result5_long, withFilter = FALSE)
writeData(wb, "result5_short_DE", result5_short, withFilter = FALSE)
writeData(wb, "result5_spread_DE", result5_spread, withFilter = FALSE)

saveWorkbook(wb, "C:/Data/Result_Excel/result5_DE.xlsx", overwrite = TRUE)
